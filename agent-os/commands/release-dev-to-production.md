...
4. If `dev` is behind `main`, repair ancestry first:
   ...
   - Fetch again and verify:
     `git merge-base --is-ancestor origin/main origin/dev`.
5. Find an existing open release PR from `dev` to `main`. If none exists, open one
   with the chosen title.
6. Ensure the release PR title is release/promote-shaped. Prefer today's local date:
   `chore(release): promote dev to main (YYYY-MM-DD)`.
...
8. If the release PR is `BEHIND`, repeat the ancestry repair step. `main` may have
...
11. When required checks are green and the PR is mergeable, **stop — do not merge**.
