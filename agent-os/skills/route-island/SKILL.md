---
name: route-island
description: Self-contained route folders with page-prefixed role files, folder-per-unit sub-components, sub-pages/ for nested routes, and URL-driven CRUD dialogs.
---
...
└── sub-pages/<child>/                 nested routes — full recursive copy of this tree
```
...
| `layout` | `<Page>Layout.tsx` (uses `<Outlet />`) | listed in `children`; disk: `sub-pages/<child>/` |
...
| Browser E2E (full app)                     | Project `tests/e2e/` (Playwright) — document flows in island `OVERVIEW.md` |
...
| `@/core/*`, `@/shared/*`, `@/lib/*`       | Sibling `sub-pages/*/api.ts` (each child owns its data)        |
| `@/tests/utils/*` from `__tests__/`       | Parent's `api.ts` from children (children fetch independently) |

## AI workflow (every new island)

1. **Read** `OVERVIEW.md` (or create it) + the parent's `<page>.page.ts` if nested.
2. **Create files** in order: `OVERVIEW.md` → `<page>.page.ts` → `<page>.contracts.ts` → `<page>.api.ts` → `<Page>Page.tsx | <Page>Layout.tsx` → `<page>.route.tsx`.
