...
| `/`                                       | Resolver — no UI: active org from `me/context` → PERSONAL `/dashboard`, TEAM `/organization/$organizationId/dashboard`, else `/onboarding` | `GET /auth/me/context`                                                                             |
...
| `/organization/$organizationId`           | Org-scoped shell — guard chain (auth → membership → status) syncs context from the URL; renders AppShell                                   | memberships + permissions                                                                          |
| `/organization/$organizationId/dashboard` | Team-org dashboard — shared `<Dashboard/>`: overview + capability-gated quick actions                                                      | `GET /auth/me/context`                                                                             |
| `/organization/$organizationId/suspended` | Blocked state for suspended / lapsed organizations                                                                                         | —                                                                                                  |
...
- Tenancy: the **active org comes from `GET /auth/me/context`** (the JWT `org` claim). It drives the `/` resolver (dual-URL: PERSONAL → root `/dashboard`, TEAM → `/organization/$organizationId/dashboard`) and the organization switcher. For the **team space**, the `$organizationId` in the URL still drives the membership/status guard + switch-on-navigation; the **personal space** (`/dashboard`) has no org param and reads the active org from the session. See `docs/reference/routing-and-tenancy.md`.
...
Other shared pieces: **FormError**, **FormField** (forms), **FullPageSpinner**, **RetryError**, **QueryBoundary** (query loading/error wrapper), **DataTable** (and pagination/toolbar/column header), **MembersTable**, **InvitationsTable**, **InviteMemberDialog**, **CreateOrganizationDialog**, **OrganizationSwitcher**, **OrganizationBadges**, **SettingsModal** (global, hash-driven), **CommandPalette** (cmdk), **PermissionGuard**. Layouts: **AuthLayout** (mounted by the pathless `auth-shell` route), **AppShell** (mounted by the `$organizationId` island); the root shell lives in `app/routes/routeTree.tsx`.
