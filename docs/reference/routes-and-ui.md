...
| `/`                                       | Resolver — no UI: last-used organization → its dashboard, else `/organization` picker, else `/onboarding`                   | `GET /api/v1/organizations`                                                                        |
...
| `/organization/$organizationId/dashboard` | Dashboard — deliberate placeholder; module is rebuilt after auth (REPLACE_WITH_MODULE)                                      | —                                                                                                  |
...
- Tenancy: `GET /api/v1/organizations` (`shared/tenancy/my-organizations.ts`) feeds the `/` resolver, the picker, the organization switcher, and the `$organizationId` membership guard. **The URL is the source of truth for organization context** — see `docs/reference/routing-and-tenancy.md`.
