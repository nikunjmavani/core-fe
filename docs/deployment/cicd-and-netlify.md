...
| **Post-merge CI** (`.github/workflows/post-merge-ci.yml`)                  | Push to `main` / `release/**`  | Single-channel release-please (ungated; CHANGELOG lint-fix + PAT-attributed auto-merge of the release PR), SBOM on releases, Netlify deploy split: `deploy-development` (`--alias dev`) on every push, `deploy-production` (`--prod`) on releases only. NO redundant re-testing — pr-ci already gated the SHA.                                                    |
...
**Build runs only on GitHub.** Netlify never runs a build; it only serves the `dist/` uploaded by post-merge CI (every `main` push → development alias; a release → production). Netlify is the current deploy target of record.
