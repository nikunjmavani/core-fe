...
| -------------------------------------------------------------------------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **PR CI** (`.github/workflows/pr-ci.yml`)                                  | PR to `main` / `release/**`           | Path-filtered parallel lanes: lint, biome, knip, format, type-check, structure/token validators, agent-os evals, unit + security Vitest, build + bundle size, gitleaks, Semgrep, Trivy IaC, dependency review, actionlint; aggregate **Quality gate**.                                                                                                            |
| **Post-merge CI** (`.github/workflows/post-merge-ci.yml`)                  | Push to `main` / `release/**`         | Single-channel release-please (ungated; CHANGELOG lint-fix + PAT probe), SBOM on releases, and `deploy-development` (`--alias development`) on every src push. **Production deploy moved to Release deploy.** NO redundant re-testing — pr-ci already gated the SHA.                                                                                              |
...
| **Preview** (`.github/workflows/preview.yml`)                              | PR to `main` / `release/**`           | Build and upload `dist/` artifact (retention 7 days).                                                                                                                                                                                                                                                                                                             |
...
- PR CI (`pr-ci.yml`) and Preview (`preview.yml`) run on PRs to `main` / `release/**`.
