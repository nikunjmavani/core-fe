# `pages/organization/$organizationId` — Org-scoped shell

Route: `/organization/$organizationId` (layout — no UI of its own at the bare path; children
render inside the shared AppShell). **The URL is the single source of truth for
...
`requireAuth` → `requireOrganizationContext($organizationId)` (malformed/unknown/non-member → 404) → children run `requireActiveOrganization` (suspended → `suspended/`) and
...
| Segment     | URL                                       | Island     |
| ----------- | ----------------------------------------- | ---------- |
| `dashboard` | `/organization/$organizationId/dashboard` | dashboard/ |
| `suspended` | `/organization/$organizationId/suspended` | suspended/ |
