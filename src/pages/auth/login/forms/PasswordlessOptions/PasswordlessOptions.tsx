 ...
 * magic link. In mock mode each completes a mock login via `/auth/callback`;
...
export function PasswordlessOptions() {
  ...
  const handleGoogleSignIn = () => {
    // REPLACE_WITH_API: redirect to backend OAuth start; mock returns via /auth/callback
  ...
  const handleMagicLink = () => {
    ...
    if (config.useMockApi) {
      setTimeout(() => navigate({ to: '/auth/callback' }), 600);
    }
  };
