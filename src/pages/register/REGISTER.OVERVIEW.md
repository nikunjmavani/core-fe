# `pages/register` — New account sign-up
...
Schemas (`registerSchema`, `RegisterInput`) live in `shared/api/auth-contracts.ts`
and are shared with `login` / `forgot-password`. The fetcher lives in `shared/api/auth-api.ts`
