...
/** These gates only read the route params (the org id). */
...
/**
 * **L2/L4 — active-org context.** Validate the `$organizationId` param, confirm
 * membership, and sync the derived store + permissions (wraps
 * {@link requireOrganizationContext}).
 */
export const resolveActiveOrg: Gate<OrgRouteCtx> = async (ctx) => {
  await requireOrganizationContext(ctx.params.organizationId ?? '');
};
...
export const requireOrgStatus: Gate<OrgRouteCtx> = (ctx) => {
  requireActiveOrganization(ctx.params.organizationId ?? '');
};
