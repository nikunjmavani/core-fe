 ...
 *
 * Usage:
 *   node scripts/github/sync-env-secrets.mjs development
 *   node scripts/github/sync-env-secrets.mjs --all
 *   node scripts/github/sync-env-secrets.mjs --all --from-config-setup
 *   node scripts/github/sync-env-secrets.mjs production --dry-run
 */
...
function secretsFromConfigSetup(environmentName) {
  ...
  const privacyKey = isDevelopment ? 'VITE_PRIVACY_POLICY_URL_DEV' : 'VITE_PRIVACY_POLICY_URL_PROD';
  ...
  return new Map(
    [
      ...
      ['NETLIFY_AUTH_TOKEN', getConfigSetupValue(CONFIG_SETUP_PATH, 'NETLIFY_AUTH_TOKEN')],
...
export function syncEnvironmentSecrets(environmentName, mode, options = {}) {
  ...
  console.log(`Source:      ${options.fromConfigSetup ? 'config.setup.env' : `.env.${environmentName}`}`);
  ...
  for (const [name, value] of secrets) {
    ...
    try {
      ...
      failures += 1;
      console.error(`  ${name}: FAILED — ${setError instanceof Error ? setError.message : String(setError)}`);
    }
  }
...
function parseArguments() {
  ...
  if (argumentsList.includes('--help') || argumentsList.includes('-h')) {
    console.log('Usage: node scripts/github/sync-env-secrets.mjs [environment...] [--all] [--from-config-setup] [--check | --dry-run]');
    process.exit(0);
  }
...
function main() {
  ...
  if (mode === 'check' && totalDrift > 0) {
    console.error(`Secret drift: ${totalDrift} key(s) in local file but missing on GitHub.`);
