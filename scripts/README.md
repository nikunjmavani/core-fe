...
└── validate/
    ...
    └── route-islands.sh       # route-island shape + OVERVIEW.md references resolve
```
...
| `pnpm run validate:structure`         | Ensures every route island has its manifest, OVERVIEW.md, and top-level UI, and that all OVERVIEW.md path references resolve on disk. | Local, `pnpm health` (Phase 9), or CI.                               |
