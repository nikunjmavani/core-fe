...
- 🔶 **FE-49** Route policy in `manifest` + default-deny — manifest gains `capability?` + `onDeny?` (`permission?` existed); `OrgCapabilityKey` moved to `core/types/permissions` (so `lib` may import it); **`gatewayFromPolicy(policy)`** composes requireSession (always — default-deny) + L5 permission + L6 capability from the policy. _Files:_ page-manifest, core/types/permissions, gateway. (`module?` field lands with FE-50; per-route manifest adoption rides the route rewire.)
...
- 🔶 **FE-59** `returnTo` consume — **login done**: LoginForm reads `search.redirect`, validates via `isSafeRedirectPath`, and navigates there post-auth (else resolver default); covered by LOGIN.OVERVIEW + redirect-safety security suite. _Files:_ LoginForm, redirect-safety. (Carrying `returnTo` through register / MFA / OAuth `/callback` — incl. surviving the provider round-trip via storage — is the remaining multi-flow plumbing.)
