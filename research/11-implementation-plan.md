...
- ✅ **FE-08** Retire URL-as-source — the `/` **root landing** reads the active org from `me/context` (not last-used storage / `listMyOrganizations`), so the session is the source of truth for where to land (FE-19/23). The team-space URL-sync (`requireOrganizationContext` resolving `$organizationSlug` → the canonical org; FE-22) is the **intended** switch-on-navigation behavior, not residual URL-as-source. _Files:_ organization-resolver, route-guards.
...
- ✅ **FE-11** Gate **L2** `hydrateContext` — `me/context` is hydrated by `establishSession` (FE-05) post-auth and by the `/` resolver **before** the org chain runs; `resolveActiveOrg` consumes it. A separate hydrate gate is unnecessary by design — the context is always present when downstream gates evaluate.
- ✅ **FE-12** Gate `resolveActiveOrg` (validate `$organizationSlug`, resolve membership by slug, sync store + perms) — `app/guards/org-gates.ts`, composed into the org-shell route. _Files:_ org-gates, routeTree.
...
- ✅ **FE-52** Deny outcomes + audit — route-access-matrix test (table-driven user-state × route-policy → reachable?, real composed gateway L1→L5→L6, 12 cells, every unauthorized path blocked) **plus per-route `onDeny` wiring**: `denyAccess()` centralizes 403 (`/unauthorized`) vs hide-as-404 (`notFound()`); `requirePermission` / `requirePermissionGate` / `requireCapabilityGate` / `gatewayFromPolicy` all thread `manifest.onDeny`. _Files:_ rbac/guards (denyAccess), require-permission, require-capability, gateway, route-access-matrix test. Verified: gate tests assert default-redirect + `onDeny:'notFound'` (isRedirect/isNotFound).
...
- ✅ **FE-22** Team space — **human-readable slug URLs**: `/organization/$organizationSlug/*` (was `$organizationId`). The org guard resolves the slug → canonical org via `findMembershipBySlug`; everything downstream still keys off the immutable `org.id`, and the org-status check compares the synced store slug. Builders / resolver / switcher / picker / create-dialog / onboarding / AppShell-nav all emit slugs (personal orgs keep root `/dashboard` — no slug). Page island renamed `$organizationId/` → `$organizationSlug/` (files + manifests + overviews). _Files:_ params (ReDoS-safe slug parser; dead id parser removed), builders, route-guards, org-gates, organization-membership (`findMembershipBySlug`), organization-resolver, routeTree, AppShell, Dashboard, the island + 6 call sites; docs + e2e URLs. Verified: 830 unit + 70 e2e — slug routing end-to-end (`org_acme` ≠ `acme`, so URL assertions catch any id/slug conflation).
...
- ✅ **FE-57** Org brand theming (optional) — engine wired end-to-end: the `me/context` org gains an optional `brandColor` (`brand_color`, nullable); `applyOrgBrand()` overrides the `--color-brand` token at the document root and `useOrgBrand()` (mounted in AppShell) applies the active org's accent on change. Gracefully no-ops until the backend sends `brand_color`. _Files:_ me-context, shared/theme/org-brand, useOrgBrand, AppShell. Verified: applyOrgBrand + useOrgBrand tests.
...
- ✅ **FE-29** Webhooks — mock-first: webhook-contracts/webhooks-api/webhook-mock-store (list/create/delete over `/tenancy/organization/webhooks`), useWebhooks hooks, and the Integrations Webhooks section (cap-gated add dialog with URL + event toggles + delete). _Files:_ webhook-\*·useWebhooks·OrganizationIntegrationsPanel. (Org-wide notification-policies need a backend contract that does not exist yet — deferred to backend integration, not blocking the FE.)
- ✅ **FE-30** Notification preferences — delivered with the notifications module (FE-65): a per-category × per-channel grid in AccountNotificationsPanel over the notification-prefs API (derived-state override pattern — no prop→state effect). _Files:_ notification-contracts (prefs), notifications-api, AccountNotificationsPanel.
...
- ✅ **FE-32** MFA enroll + passkeys — **MFA** (begin/confirm/disable over `/auth/me/mfa`, useMfa, two-factor card: secret → 6-digit code → recovery codes; confirmed disable) **and passkeys** (passkey contracts/store/api over `/auth/me/passkeys`, usePasskeys hooks, PasskeysCard: list + named-register dialog + per-row revoke; REPLACE*WITH_API marks where the live path runs the WebAuthn ceremony). \_Files:* mfa-\*·passkey-\*·useMfa·usePasskeys·AccountSecurityPanel. Verified: 11 passkey tests + MFA flow.
- ✅ **FE-33** Org general — **rename** (`updateOrganization` partial name/logo PATCH, useUpdateOrganization, General panel: controlled name draft + read-only slug) **and logo** (`logoUrl` on the org contract, OrgLogoCard: preview + image-only/<1 MB upload → data URL / live `logo_url` + remove). Cap-gated on canManageMembers. _Files:_ my-organizations·useUpdateOrganization·OrganizationGeneralPanel. Verified: rename + logo upload/remove tests.
...
- ✅ **FE-38** Account panels — Security (MFA + passkeys, FE-32), Sessions (FE-31), Notifications (FE-65), Profile, and Appearance, each cap/state-aware. _Files:_ src/shared/components/SettingsModal/account/Account{Security,Sessions,Notifications,Profile,Appearance}Panel.
...
- ✅ **FE-41** e2e capstone — notifications + dual-URL org-switching e2e (atop the auth / navigation / settings / responsive suites); onboarding persists job title via `persistOnboardingResult` → `authApi.updateProfile`. _Files:_ tests/e2e/notifications.e2e, org-switching.e2e; OnboardingPage.
- ✅ **FE-42** Docs / memory ripple — routing-and-tenancy + routes-and-ui + CLAUDE.md + island overviews track the dual-URL/slug design; the `core-fe-be-integration-plan` memory records the as-built status. _Files:_ docs/reference/\*, CLAUDE.md, memory.
