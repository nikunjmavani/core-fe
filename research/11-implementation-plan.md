...
- 🔶 **FE-08** Retire URL-as-source — the `/` **root landing** now reads the active org from `me/context` (not last-used storage / `listMyOrganizations`), so the session is the source of truth for where to land (FE-19/23). The org-shell guard (`requireOrganizationContext`) syncs from the `$organizationSlug` URL for the team space (the slug resolves to the canonical org; FE-22); fully retiring that URL-sync is the broader FE-12 gate work. _Files:_ organization-resolver (done), route-guards (slug-based).
