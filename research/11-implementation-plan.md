...
- 🔶 **FE-59** `returnTo` consume — **login + MFA done**: `isSafeRedirectPath`/`safeRedirect` promoted to `shared/auth/redirect-safety`; LoginForm validates `search.redirect` and carries it to `/mfa` via router state; MfaForm honors it post-verify (else resolver default). Covered by the redirect-safety security suite + colocated unit. _Files:_ shared/auth/redirect-safety, LoginForm, MfaForm. (register / OAuth-`/callback` round-trip via storage is the remaining lower-value plumbing.)
